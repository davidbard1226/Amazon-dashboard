...
echo "[Build] Installing Chrome Headless Shell into local cache..."
npx @puppeteer/browsers install chrome-headless-shell@stable --path "$(pwd)/.cache/puppeteer"
...
ls -R "$(pwd)/.cache/puppeteer" | grep "chrome" || echo "[Build] Warning: Browser not found!"
